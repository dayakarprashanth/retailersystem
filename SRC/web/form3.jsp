     ...
     <center>
        ...
        <table class="ali">
            <%
                ...
                String hg="";
                String wg="";
                PreparedStatement p2=con.prepareStatement("select * from doorheight where door_height_size='"+h+"' ");
                ResultSet r2=p2.executeQuery();
                if(r2.next()){
                    hg=r2.getString("door_height_group");
                    session.setAttribute("hg",hg);
           ...
           <tr>
               <td>Panel Name</td>
               <td><select name="panel">
                       <option selected="" disabled="">Choose one</option>
                       <%
                               
                            PreparedStatement p1=con.prepareStatement("select * from doorwidth where door_width_size='"+w+"' ");
                            ResultSet r1=p1.executeQuery();
                            while(r1.next()){
                                 wg=r1.getString("door_width_group");

                                PreparedStatement p3=con.prepareStatement("SELECT panel_name FROM paneltype WHERE panel_id IN (SELECT panel_id FROM product WHERE door_width_group='"+wg+"' AND door_height_group='"+hg+"') ");
                                System.out.println(p3);
                                ResultSet r3=p3.executeQuery();
                                while(r3.next()){
                       %>
                       <option value="<%=r3.getString("panel_name")%>"><%=r3.getString("panel_name")%></option><%}}%>
                   </select></td></tr>
